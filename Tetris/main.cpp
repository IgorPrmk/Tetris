...
void initMenuItems()
{
	...
	if (g_game.m_inProgress || g_game.m_isGameOver || g_game.m_isEnteringName)
	{
...
{
	if (INDEX < 0 || INDEX >= g_menuItems.size()) //якщо індекс пункту меню не в межах списку
	{
		//то нічого не робимо
		return;
	}
...
{
	if (INDEX < 0 || INDEX >= g_settingsItems.size()) //якщо індекс пункту налаштувань не в межах списку
	{
		//то нічого не робимо
		return;
	}
...
{
	if (INDEX < 0 || INDEX >= g_controlsItems.size()) //якщо індекс пункту управління не в межах списку
	{
		//то нічого не робимо
		return;
	}
...
int WINAPI WinMain(HINSTANCE programHandle, HINSTANCE /*для сумісності*/, LPSTR commandLine, int windowType)
{
	...
	if (!RegisterClassEx(&windowSettings)) {
	...
	{
		wchar_t buf[128];
		swprintf(buf, 128, L"CreateWindowEx FAILED, GetLastError = %u", GetLastError());
