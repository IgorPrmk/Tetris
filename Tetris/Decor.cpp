...
void Decor::updateDecorRain()
{
	for (auto& currentDecoreFigure : g_decorFigures)
	{
		...
		if (currentDecoreFigure.y > g_WINDOW_HEIGHT + 40)
		{
			...
			//генерація випадкового обертання фігури від 0 до 3
			const int RANDOM_ROTATION = g_random->getRandomInt(0, 3);
			currentDecoreFigure.figure.rotateFigure(RANDOM_ROTATION);
			//==========================================================

			//випадкова позиція вище екрану ============================
			currentDecoreFigure.x = g_random->getRandomDouble(0.0, g_WINDOW_WIDTH);
			currentDecoreFigure.y = g_random->getRandomDouble(-g_WINDOW_HEIGHT, 0.0);
			//==========================================================

			//випадкова швидкість фігури
			currentDecoreFigure.speed = g_random->getRandomDouble(1.5, 2.5);
		}
	}
}
